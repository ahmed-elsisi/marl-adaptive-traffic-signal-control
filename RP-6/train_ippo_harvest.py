...
def train(config_path: str, num_iterations: int, checkpoint_freq: int, resume: str = None, env_seed: int = None):
    config = load_config(config_path)

    # Multi-seed driver overrides env seed via CLI; falls back to YAML's value.
    if env_seed is not None:
        config.setdefault("env_config", {})["seed"] = env_seed
...
def main():
    ...
    try:
        train(args.config, args.iterations, args.checkpoint_freq, args.resume, env_seed=args.seed)
        print("\nTraining complete.")
