...
Your action space:
- Action 0: NS through + right (Phase 0: GGrgrrGGrgrr)
- Action 1: NS left turn (Phase 6: GrGGrrGrGGrr)
- Action 2: EW through + right (Phase 2: GrrGgrGrrGGr)
- Action 3: EW left turn (Phase 4: GrrGrGGrrGrG)

Key fixes:
1. ✓ Action space: Discrete(4) - matches your setup
2. ✓ Index-based phase setting via setPhase (not strings)
...
class SUMOTrafficEnv(MultiAgentEnv):
    ...
    Uses SUMO's phase indices from marl-proj.ttl.xml for correct phase display.
    """
    ...
    def __init__(self, env_config: Dict):
        ...
        # Track current phase for each agent (for phase holding)
        self.current_phases = {}
    ...
    def _apply_actions(self, action_dict: Dict[str, int]):
        """
        Apply traffic signal actions using phase indices.
        
        CORRECTED ACTION MAPPING (based on your marl-proj.ttl.xml):
        ...
        This uses SUMO's phase indices from your ttl.xml file.
        """
        for agent_id, action in action_dict.items():
            try:
                # Map action to phase index from your ttl.xml
