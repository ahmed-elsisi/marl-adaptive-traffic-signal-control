...
class MAPPORewardFunction:
    ...
    def __init__(
        ...
        # Throughput tracking
        self.prev_departed_count = {agent_id: 0 for agent_id in agent_ids}
    ...
    def _calculate_throughput_bonus(self, agent_id: str, traci_conn) -> float:
        ...
        Throughput = vehicles that departed (completed trip) this step.
        Bonus encourages moving traffic through the network.
        """
        try:
            current_departed = traci_conn.simulation.getDepartedNumber()
            delta_departed = current_departed - self.prev_departed_count[agent_id]
            self.prev_departed_count[agent_id] = current_departed
            
            # Normalize by expected throughput per agent per step
            # Assume ~1-5 vehicles per agent per 5-second step
            normalized_throughput = delta_departed / (len(self.agent_ids) * 5.0)
        except:
    ...
    def _calculate_pressure_penalty(self, agent_id: str, traci_conn) -> float:
        ...
        edge_config = self.edge_mappings.get(agent_id, {})
        # print(f"Edge config for {agent_id} is {edge_config}")
        ...
        for edge_id in edge_config.get('incoming', []):
            ...
            # print(f"Incoming Edge ID: {edge_id} for {agent_id} count {incoming_count}")
        ...
        for edge_id in edge_config.get('outgoing', []):
            ...
            # print(f"Outgoing Edge ID: {edge_id} for {agent_id} count {outgoing_count}")
        ...
        max_pressure = len(edge_config.get('incoming', [])) * 3 * self.norm_config['pressure_max']
        
        normalized_pressure = positive_pressure / max_pressure if max_pressure > 0 else 0.0
        
        # print(f"Pressure for {agent_id}: {pressure}, positive: {positive_pressure}, max: {max_pressure}, normalized: {normalized_pressure}")
    ...
    def reset(self):
        ...
        self.prev_departed_count = {agent_id: 0 for agent_id in self.agent_ids}  # ✅